...
if (isset($_POST['submit'])) {
   ...
   // $dob = mysqli_real_escape_string($db->link, $_POST['dob']);
    ...
    $password = sha1(mysqli_real_escape_string($db->link, $_POST['password']));
    
   
    
    
//    $query = "INSERT INTO user(username,email,phn,password)    ///added since every time it shows the message Field must not be Empty though inserted all.
  // Values('$name','$email','$phn', '$password')";     /// but now unknown...
    //        $create = $db->insert($query);
    
    
    
    
    if ($name == '' || $email == '' || $phn == '' || $password == '' || $username == '') {        //$dob == '' || $userid == '' ||   '$dob', '$userid',    dob,username,
        ...
        if ($user_exist) {
            ...
            $query = "INSERT INTO user(username, email,phn,password, name) 
   Values('$username','$email','$phn', '$password', '$name')";
            $create = $db->insert($query);
            if ($create) {
